	...
	while (1) {
		...
		switch(choice) {
			...
			case 1: // ADD PRODUCT
				...
				view.promptForInt("Lifespan (only used for perishables)", ls);
				view.promptForInt("1 -> Bakery, 2 -> CoffeeTea, 3 -> Dairy, 4 -> Meat, 5 -> MiscGoods", typ);
				// would use function pointer if had more time
				switch(typ) {
					case 1:
						prod = new Bakery(name, size, units, price, y, m, d, ls);
						break;
					case 2:
						prod = new CoffeeTea(name, size, units, price, y, m, d);
						break;
					case 3:
					...
					case 4:
						...
						break;
					default:
						prod = new MiscGoods(name, size, units, price, y, m, d);
						break;
				}
				store.addProd(prod);
				view.pause();
...
// order this thing by expiry date :@
void InvControl::initProducts() {
	store.addProd(new MiscGoods("Peachy Laundry Soap", "2 L", 3, 8.99f, 2017, 5, 15));
	store.addProd(new Meat("Daisy's Spicy Chili", "150 g", 5, 1.29f, 2017, 5, 15, 69));
	store.addProd(new MiscGoods("Daisy's Maple Baked Beans", "220 g", 2, 2.49f, 2017, 5, 15));
	...
	store.addProd(new CoffeeTea("Lightening Bolt Instant Coffee", "150 g", 8, 4.99f, 2017, 5, 15));
	store.addProd(new CoffeeTea("Lightening Bolt Decaf Coffee", "100 g", 2, 4.99f, 2017, 5, 15));
	store.addProd(new MiscGoods("Munchies BBQ Chips", "250 g", 7, 2.99f, 2017, 5, 15));
	store.addProd(new MiscGoods("Munchies Ketchup Chips", "250 g", 3, 2.99f, 2017, 5, 15));
	store.addProd(new MiscGoods("Dogbert Salted Chips", "210 g", 4, 1.99f, 2017, 5, 15));
	store.addProd(new MiscGoods("Dogbert Sweet and Spicy Popcorn", "180 g", 5, 2.29f, 2017, 5, 15));
	store.addProd(new MiscGoods("Sudzzy Dish Soap", "1 L", 10, 3.99f, 2017, 5, 15));
}
